...
let currentSwapTokenA = 1000000000;
let currentSwapUSDC= 1000000000;
...
console.log(SWAP_PROGRAM_OWNER_FEE_ADDRESS)
...
export async function createTokenSwap(
 ...
 // const payer = await newAccountWithLamports(connection, 1000000000);
  ...
  owner = new Account([30,72,146,239,125,42,31,114,39,254,222,19,107,128,227,49,72,67,15,179,200,44,196,47,113,215,87,244,166,62,9,64,
    235,22,239,76,86,188,196,67,204,110,174,27,101,236,214,15,167,127,87,199,134,29,225,207,153,255,77,181,21,103,153,190]);
...
//   //Here is the code we are creating mint. In this we are adding details of payer
  tokenPool = await Token.createMint(
   ...
   console.log('creating token A');
    ...
    const ownerwallet = new Uint8Array([30,72,146,239,125,42,31,114,39,254,222,19,107,128,227,49,72,67,15,179,200,44,196,47,113,215,87,244,166,62,9,64,
      235,22,239,76,86,188,196,67,204,110,174,27,101,236,214,15,167,127,87,199,134,29,225,207,153,255,77,181,21,103,153,190]);
    const mintA = new PublicKey("7d7i23QEdCP2kvVfJm3qAyJmNFiHNpNV5k7DupYMkEYM");
  // console.log('--------------------------------SOLANA_TESTNET_USDC--------------------------------');
   console.log(mintA);
  ...
  /*(async () => {
    ...
    let fromWallet = web3.Keypair.fromSecretKey(ownerwallet);
...
// //code to get USDC Account account Info
  ...
  /*(async () => {
    ...
    var connection = new web3.Connection(web3.clusterApiUrl("testnet"));
    // Construct wallet keypairs
    var fromWallet = web3.Keypair.fromSecretKey(ownerwallet);
    var toWallet = web3.Keypair.generate();
  ...
  //   // Create associated token accounts for my token if they don't exist yet
  var fromTokenAccount = await mintA_Token.getOrCreateAssociatedAccountInfo(
    fromWallet.publicKey
  )
  var toTokenAccount = await mintA_Token.getOrCreateAssociatedAccountInfo(
    toWallet.publicKey
  )
  ...
  // console.log("tokenAccountBBalanceAfterMint address: ", tokenAccountBBalanceAfterMint);
   ...
   const ownerwallet1 = new Uint8Array([30,72,146,239,125,42,31,114,39,254,222,19,107,128,227,49,72,67,15,179,200,44,196,47,113,215,87,244,166,62,9,64,
    235,22,239,76,86,188,196,67,204,110,174,27,101,236,214,15,167,127,87,199,134,29,225,207,153,255,77,181,21,103,153,190]);
   const mintB = new PublicKey("HX3MWTYt3qaZ7RaXk2KiXzWGhoaCTo5gaGzY7jaedHXi");
  // console.log('--------------------------------SOLANA_TESTNET_USDC--------------------------------');
  console.log(mintB);
  // //console.log("[SOLANA_TESTNET_USDC_PUBLEY.toBuffer()] :"+[SOLANA_TESTNET_USDC_PUBKEY.publicKey]);
...
// //code to get USDC Account account Info
  ...
  /*(async () => {
    ...
    var connection = new web3.Connection(web3.clusterApiUrl("testnet"));
    // Construct wallet keypairs
    var fromWallet = web3.Keypair.fromSecretKey(ownerwallet1);
    var toWallet = web3.Keypair.generate();
  ...
  console.log("fetchedTokenSwap.mintA: "+fetchedTokenSwap.mintA+" :mintA.publicKey :"+mintA)
  console.log("fetchedTokenSwap.USDC: "+fetchedTokenSwap.mintB+" :USDC :"+mintB)
...
//   assert(fetchedTokenSwap.USDCAccount.equals(USDCAccount));
//   assert(fetchedTokenSwap.mintA.equals(mintA.publicKey));
//   assert(fetchedTokenSwap.USDC.equals(USDC));
